...
export default function AdminSubjectsListPage() {
  ...
  const remove = async (item) => {
    if (!window.confirm(`Удалить дисциплину ${item.code} — ${item.name}?`)) return;
    setSaving(true);
    setError('');
    try {
  ...
  return (
    <Stack>
      <Group justify="space-between">
        <Title order={2}>Дисциплины</Title>
        <Button component={Link} to="/admin/subjects/new">Добавить дисциплину</Button>
      </Group>
      {loading && <Loader color="teal" />}
      {error && <Alert color="red">{error}</Alert>}
      <ListControls search={search} onSearchChange={setSearch} searchPlaceholder="Код или название" />
      <Card withBorder>
        <Table striped>
          <Table.Thead><Table.Tr><Table.Th>Код</Table.Th><Table.Th>Название</Table.Th><Table.Th>Действия</Table.Th></Table.Tr></Table.Thead>
          <Table.Tbody>
            {visible.map((item) => (
              <Table.Tr key={item.id}>
                ...
                <Table.Td>
                  <Group gap="xs">
                    <Button size="xs" variant="light" component={Link} to={`/admin/subjects/${item.id}/edit`}>Редактировать</Button>
